...
import { Database, studioAliases } from "@tableland/sdk";
...
export const handler = async (
  ...
  try {
    const { chain, providerUrl, apiUrl, store, table, path } = argv;
    const store = new FileStore(store as string);
    const projectId = getProject({ store, ...argv});
    ...
    const chain = getChainFromTable(table, projectId);
    ...
    const db = new Database({
      signer: wallet,
      aliases: studioAliases(projectId, apiUrl)
    });
...
export const handler = async (argv: Arguments<Options>): Promise<void> => {
  ...
  try {
    ...
    if (file != null) {
      statement = readFileSync(file, { encoding: "utf-8" });
